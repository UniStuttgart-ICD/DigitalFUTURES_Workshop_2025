...
def main(robot_ip: str = "192.168.56.101") -> None:
    ...
    try:        # Get current joint positions
        ...
        target_pose[2] += 0.05  # Move up 150mm in Z-axis
        rtde_c.moveL(target_pose, 0.1, 1.0)  # Speed: 0.1 m/s, Acceleration: 1.0 m/s² (home speeds)
        time.sleep(1.0)
          # Move back to original position
        console.print("[yellow]Moving back to original position...[/yellow]")
        rtde_c.moveL(actual_tcp_pose, 0.1, 1.0)  # Speed: 0.1 m/s, Acceleration: 1.0 m/s² (home speeds)
        time.sleep(1.0)
